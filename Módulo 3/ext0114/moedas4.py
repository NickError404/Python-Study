...
def resumo(date=0):
    resuma = dict()
    resuma['Aumentar'] = moeda(aumentar(date))
    resuma['Dimiuir'] = moeda(diminuir(date))
    resuma['dobro'] = moeda(dobro(date))
    resuma['metade'] = moeda(metade(date))
    print('-' * 30)
    for k, v in resuma.items():
        print(f'O valor de {k} é de {v}'.center(30))
    print('-' * 30)
